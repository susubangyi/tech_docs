...
## Ansible를 적극 활용하자
빅데이터, AI, IoT, 클라우드를 융합하는 것이 기본이 되는 요즘과 같은 시대에서는 대규모의 클러스터를 운영하는 것이 필수적으로 요구되고 있습니다.  대규모의 클러스터 환경을 구성한다는 것은 너무나도 많은 시간을 소비하는 작업입니다. 그러한 대규모 클러스터 환경에서 Ansible과 같은 도구는 서비스 배포 및 관리를 위하여 높은 편의성을 제공합니다. 단순하게 개인 연구 목적으로 작은 클러스터를 운영한다면 굳이 필요 없을 수도 있겠으나 언젠가는 대규모의 클러스터를 운영하는 순간이 올 것이기에 Ansible과 같은 도구에 익숙하도록 미리 준비하는 스마트한 개발자가 되었으면 좋겠습니다.
